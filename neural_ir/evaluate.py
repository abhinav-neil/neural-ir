...
ce_parser.add_argument("--max_length",
                       type=int,
                       default=512,
                       help="Maximum lenght of a query + doc pair")
######################################################################
#              Dense Encoder argument parser                         #
######################################################################
dense_parser = sub_parsers.add_parser("dense", help="Dense Model")
dense_parser.add_argument("--query_max_length", type=int, default=100, help="Maximum query length")
dense_parser.add_argument("--doc_max_length", type=int, default=250, help="Maximum document length")
######################################################################
#              Sparse Encoder argument parser                         #
######################################################################
sparse_parser = sub_parsers.add_parser("sparse", help="Sparse Model")
sparse_parser.add_argument("--query_max_length", type=int, default=100, help="Maximum query length")
sparse_parser.add_argument("--doc_max_length",
                           type=int,
                           default=250,
...
if args.model == "ce":
    ...
    model = CrossEncoder.from_pretrained(OUTPUT_DIR).to(args.device)
elif args.model == "dense":
    triplet_collator = BiEncoderTripletCollator(tokenizer,
                                                args.query_max_length,
                                                args.doc_max_length)
    pair_collator = BiEncoderPairCollator(tokenizer, args.query_max_length, args.doc_max_length)
    model = DenseBiEncoder.from_pretrained(OUTPUT_DIR).to(args.device)
elif args.model == "sparse":
    triplet_collator = BiEncoderTripletCollator(tokenizer,
                                                args.query_max_length,
                                                args.doc_max_length)
    pair_collator = BiEncoderPairCollator(tokenizer, args.query_max_length, args.doc_max_length)
    model = SparseBiEncoder.from_pretrained(OUTPUT_DIR).to(args.device)
else:
    raise Exception("Invalid selection. Select either {ce, dense, sparse} model for training.")
...
training_args = TrainingArguments(
    output_dir='./', # Nothing will be saved so it doesnt matter
    fp16=True,
)
######################################################################
